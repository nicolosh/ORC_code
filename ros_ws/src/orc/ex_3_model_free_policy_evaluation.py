...
def policy_pendulum(env, x): # policy that tries to make the pendulum reach the top 
    ''' The policy to be evaluated (fixed policy as when did policy evaluation with DDP) '''
...
if __name__=="__main__":
    ...
    if(env_name == 'pendulum'):
        ...
        N_EPISODS         = 1000    # number of episodes used for evaluating the policy
        MAX_EPISOD_LENGTH = 100     # max length of an episode
        LEARNING_RATES    = [0.5, 1.0]     # TD0 learning rates
        CONVERGENCE_THR   = 1e-5    # convergence threshold of policy evaluation for computing the real/ground truth value function with DynProgr
    ...
    V_mc, err_mc = mc_policy_eval(env, DISCOUNT, policy, N_EPISODS, MAX_EPISOD_LENGTH,
                       V_real, DO_PLOTS, NPRINT) # evaluates the policy using MC
    ...
    for i,alpha in enumerate(LEARNING_RATES):
        ...
        V_td[i], err_td[i] = td0_policy_eval(env, DISCOUNT, policy, V, N_EPISODS, 
                                        MAX_EPISOD_LENGTH, V_real, alpha, DO_PLOTS, NPRINT) #  evaluates the policy using TD0
    
    # the learning rates are function basically also of the noise in the environment:
    # if the noise is low, is correct
    # to keeo high values for learning rates and viceversa.
    
    plt.figure()
    # error for value function estimation over time for the different methods
