...
Actor-Critic Algorithm: we have both the policy evaluation and policy improvement steps
The policy used is always an epsilon greedy policy

SARSA and Q learning are really an extension of policy iteration and value iteration which 
are 2 specific instances of generalized policy iteration
'''
...
### --- Hyper paramaters
NITER                   = 10            # number of iterations of the algorithm for updating the policy (policy evaluation phase)
NEPISODES               = 500           # number of training episodes where we update the Q function (= how many simulations we run for each algorithm iteration)--> policy improvement
...
pi = env.c2du(0.0)*np.ones(env.nx, np.int64)   # policy
...
print("\nCompute real Value function of greedy policy found by SARSA")
...
'''
Since  we  are minimizing the algorithm which outputs the most negativee value 
as average value is better!!!
'''