...
class Integrator:
    ...
    def integrate(self, ode, x_init, U, t_init, dt, ndt, N, scheme):
        ''' Integrate the given ODE and returns the resulting trajectory:
            ...
            - dt: the time step of the trajectory (it is the time step not of the integration scheme but of 
                  the control input U which is discretized by dt )
            - ndt: the number of inner time steps for each time step  (ndt = time step for the integration scheme)
                   is ndt = 1 it means that integration scheme and the control input U are discretized with same time step,
                   if it's 2, for 1 control input value we take 2 integration steps): will be used to
                   get a greater accuracy of the integration scheme without having to discretize the U at each time
                   So at ndt gets larger the time step gets smaller!
    ...
    def integrate_w_sensitivities_u(self, ode, x_init, U, t_init, dt, N, scheme):
        ...
        for i in range(N):
            ...
            iu, iu1 = i*nu, (i+1)*nu
            #SENSITIVITIES OF EXPLICIT EULER
...
#    system = 'stiff-diehl'
    ...
    integrators = []
    integrators += [{'scheme': 'RK-4',      'ndt': 1000}]    # used as ground truth: NDT IS very small putting high numbers so the accuracy is very large
