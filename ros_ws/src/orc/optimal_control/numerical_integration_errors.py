...
"""
When the time step is not sufficiently small may be that lower order runge-kutta methods are better than higher ones
instead typically for sufficienlty small time steps is always the case that higher order methods are the best ones!
"""
...
if __name__=='__main__':
    ...
    ''' Compute errors of different integration schemes w.r.t the baseline got by using very small time step: 
        compute the integration of same dynamics several times
        using different integrations schemes and time steps
        
        N.B. the comparison against ndt is NOT FAIR instead the one w.r.t #functionCalls yes!
    '''
