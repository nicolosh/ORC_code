...
class ODE:
    ...
    ''' returns the dynamics dx/dt (x_dot = f(x,u,t))'''
    def f(self, x, u, t): 
...
class ODELinear(ODE):
    ''' A linear ODE: dx = A*x + B*u + b (offset)
    '''
    ...
    def f(self, x, u, t, jacobian=False):
        ...
        '''A = dx_dot/dx and B = dx_dot/du: used to compute the 
        sensitivities of our scheme'''
        if(jacobian):
...
class ODERobot(ODE):
    ...
    def f(self, x, u, t, jacobian=False):
        ...
        else: #calculate the accelerations of joints
...
#            pin.computeABADerivatives(model, data, q, v, u)
            # S.S.R
            # the following are used to compute the sensitivities
            self.Fx[:nv, :nv] = 0.0
            self.Fx[:nv, nv:] = np.identity(nv)
    ...
    #DEBUGGING USING FINITE DIFFERENCES (SLOWER) -----------------
