...
def backward_pass(solver, X_bar, U_bar, mu):
        ...
        #we take the last time step with -[1,:]
        solver.l_x[-1,:]  = solver.cost_final_x(X_bar[-1, :]) #X_bar matrix N(time-step-dim)xn(state-dim)
        solver.l_xx[-1,:,:] = solver.cost_final_xx(X_bar[-1, :])
        V_xx[N,:,:] = solver.l_xx[-1,:,:] #[tim_index, row_index, coloumn_index]
        V_x[N,:]    = solver.l_x[-1,:] # gradient has [time index, element_index]
        ...
        for i in range(N-1, -1, -1):
            ...
            V_x[i,:]    = solver.Q_x[i,:] + \
                          solver.K[i,:,:].T @ (solver.Q_u[i,:] + solver.Q_uu[i,:,:] @ solver.w[i,:]) + \
                          solver.Q_xu[i,:,:] @ solver.w[i,:]
            V_xx[i,:]   = solver.Q_xx[i,:,:] + solver.K[i,:,:].T @ solver.Q_uu[i,:,:] @ solver.K[i,:,:] + \
                          solver.Q_xu[i,:,:] @ solver.K[i,:,:] + solver.K[i,:,:].T @ solver.Q_xu[i,:,:].T
