...
"""
.
Derived class of DDPSolver implementing NON-linear system dynamics specifically for a manipulator
.
...
class DDPSolverManipulator(DDPSolverLinearDyn):
    ''' 
        ...
        plus a control regularization: 
            ...
            The task is put the robotic arm at pi/2 from pi = 0 (horizontal position of the arm)
    '''
    ...
    def f(self, x, u):
        ...
        q = x[:nq]
        v = x[nq:]
        ddq = pin.aba(model, data, q, v, u) # for computing the forward dynamics of a MBS
        self.dx[:nv] = v
        self.dx[nv:] = ddq
    ...
    def start_simu(self, X, U, K, dt_sim): #simulate the systems with time_step 
                                           #dt_sim smaller than the time used for 
                                           #discretizing the O.C.P (to be more accurate)
        t = 0.0
        simu = self.simu
...
if __name__=='__main__': # creating the problem and using the solver, running everything
    ...
    ddp_params = {}
    ddp_params['alpha_factor'] = 0.5 #reducing alpha at each iteration
    ddp_params['mu_factor'] = 10. #increase or decrease mu
    ddp_params['mu_max'] = 1e0 #max_value for mu to give up and stop
    ddp_params['min_alpha_to_increase_mu'] = 0.1 #min_alpha so that we still increase mu
    ddp_params['min_cost_impr'] = 1e-1 #so 10% of expected cost improvement
    ddp_params['max_line_search_iter'] = 10
    ddp_params['exp_improvement_threshold'] = 1e-3 #for deciding if we reached convergence (if exp_improvement is below --> converged)
    ...
    x_tasks = np.concatenate((conf.qT, np.zeros(robot.nv)))  # goal state or desired configuration
    N_task = N;                     # time step to reach goal state
    
    tau_g = robot.nle(conf.q0, np.zeros(robot.nv))
    for i in range(N):
        U_bar[i,:] = tau_g #initial guess = gravity compensation torques
    
    ''' TASK COST FUNCTION  '''
    lmbda = 1e-3           # modified FROM 1e-2 TO 1e-3: control weight regularization effort for the cost
    H_xx = np.zeros((N+1, n, n));
    h_x  = np.zeros((N+1, n));
    h_s  = np.zeros(N+1);
    W = np.diagflat(np.concatenate([np.ones(nq), np.zeros(nv)]))
    #cost = W_*||x-x_task||^2 , W_ = weight
