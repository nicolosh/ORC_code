...
class DDPSolver:
    ...
    ''' Simulate system forward in time with computed linear feedback control law '''
    def simulate_system(self, x0, U_bar, K, X_bar):
        ...
        X[0,:] = x0
        for i in range(N):
            #X_bar and U_bar: state and control reference trajectory
    ...
    def update_expected_cost_improvement(self):
        ...
        for i in range(self.N):
            self.d1 += self.w[i,:].T @ self.Q_u[i,:] #w feedforward control input computed in backward pass
    ...
    '''
    def solve(self, x0, U_bar, mu):    #U_bar: initial guess for the 
        ...
        for j in range(self.max_iter):
            ...
            cst = self.cost(X_bar, U_bar) # baseline (for nominal cost)
...
#                print("Expected improvement", exp_impr, "Real improvement", new_cost-cst)
                ...
                # we are not accepting variations that makes the cost increase
                if(exp_impr > 0.0): # expected cost improvement POSITIVE not negative we set it to a negative number as it should be
                    exp_impr = -1.0
                    
                relative_impr = (new_cost-cst)/exp_impr
                
                # real improvement = cost now - cost at previous iteration
                if(relative_impr > self.min_cost_impr):
                    ...
                    U_bar += alpha*self.w
                    cst = new_cost
                    break # exiting from for
                    
                alpha = self.alpha_factor*alpha
            
            if(not line_search_succeeded): #no improvement
                mu = mu*self.mu_factor #we need to recompute whole backward pass
                print("No cost improvement, increasing mu to", mu)
                if(mu>self.mu_max):
                    print("Max regularization reached. Algorithm failed to converge.")
                    converged = True
            else: # increasing or decreasing mu based on alpha which is based on the relative cost improvement
                ...
                if(alpha>=self.min_alpha_to_increase_mu):
                    ...
                    if(mu>self.mu_max):
                        print("Max regularization reached. Algorithm failed to converge.")
                        converged = True
                self.callback(X_bar, U_bar) #visualizing the robot trajectories in the viewer
        ...
        # compute nominal state trajectory X_bar
        (X_bar, U_bar) = self.simulate_system(x0, U_bar, self.K, X_bar)
        return (X_bar, U_bar, self.K)  # return the optimal values for
                                      # state, control trajectory and local feedback gains
