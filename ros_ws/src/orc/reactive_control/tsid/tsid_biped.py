...
class TsidBiped:
    ''' Standard TSID formulation for a biped robot standing on its rectangular feet.
        - Center of mass task (control the robot CoM to be in a certain point)
        - Postural task
        - 6d rigid contact constraint for both feet (it's a surface on a surface contact--> our foot can neither move nor rotate in any direction): a 3D contact is 
        basically a point contact: we cannot move but rotate.
    ...
    def __init__(self, conf, viewer=pin.visualize.MeshcatVisualizer):
        ...
        # contact points for creating the feet of the robot:
        # the bigger the feet the more further away will be the points (we are catching
        # the 4 vertices of a rectangle (any other CONTACT point in the middle of the rectangle will be a convex combination of the previous 4 --> will be 
        # REDUNDANT )))
        ...
        copTask = tsid.TaskCopEquality("task-cop", robot) # CoP = centre of pressure task for controlling
        # the center of pressure at the feet: it's the weighted average of contact points where we assign higher weights to points where we have higher forces!
        
        formulation.addForceTask(copTask, conf.w_cop, 1, 0.0)
        
        amTask = tsid.TaskAMEquality("task-am", robot) # Angular momentum task for the robot: used for balancing to maintein L = 0
        ...
        postureTask = tsid.TaskJointPosture("task-posture", robot) # joint angles task or postural task
